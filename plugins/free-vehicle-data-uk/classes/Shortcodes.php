...
class Shortcodes{
    ...
    public function __construct() 
    {
        ...
        add_shortcode('fvd_returnmotrecord',[$this,'GetMOT2']);

    }
    public function GetMOT2($atts)
    {
        global $json_return;

        $CurrentRecord = ($atts["record"]);
        if (isset ($json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['MotResultsSummary'])){
            
            $aMotHistory = $json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['MotResultsSummary']['MotResults'];
            if(!isset($aMotHistory[$CurrentRecord]['OdometerModel']['OdometerReading'])){
                return '';
            }

            $completiondate = date('d M Y', strtotime($aMotHistory[$CurrentRecord]['DateOfTest'])); 
            $testresult = ($aMotHistory[$CurrentRecord]['Result']?'PASSED':'FAILED'); // PASSED OR FAILED
            $expiryDate =  (isset($aMotHistory[$CurrentRecord]['TestExpiryDate']))?date('d M Y', strtotime($aMotHistory[$CurrentRecord]['TestExpiryDate'])):'Not Available';
            
            $odometerValue = ($aMotHistory[$CurrentRecord]['OdometerModel']['OdometerReading']); //e.g. 51200
            $odometerUnit  = ($aMotHistory[$CurrentRecord]['OdometerModel']['OdometerFormat']); //mi or km

            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['0']))){
                $RfaComments = '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['0']) . '</p>';
            }else{
                $RfaComments = '<p>Nothing to display.</p>';	
            }
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['1']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['1']) . '</p>';
            }		
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['2']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['2']) . '</p>';
            }		
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['3']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['3']) . '</p>';
            }		
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['4']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['4']) . '</p>';
            }		
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['5']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['5']) . '</p>';
            }		
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['6']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['6']) . '</p>';
            }									
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['7']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['7']) . '</p>';
            }
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['8']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['8']) . '</p>';
            }
            if (isset(($aMotHistory[$CurrentRecord]['RefusalReasons']['9']))){
            $RfaComments = $RfaComments . '<p><img src="' . plugin_dir_url(FVD_FILE) . 'assets/images/alert.png" width="20" height="15"> ' . ($aMotHistory[$CurrentRecord]['RefusalReasons']['9']) . '</p>';
            }
    ...
    public function CallJson($atts)
    {
        ...
        if (isset($_GET['Reg'])) {
            ...
            $json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['TotalMotRecords'] = 0;
            $json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['LastMotDate'] = 'Not Available';
            if(isset($json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['MotResultsSummary']['MotResults'])){
                if(is_array($json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['MotResultsSummary']['MotResults'])){
                    $json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['TotalMotRecords'] = sizeof($json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['MotResultsSummary']['MotResults']);
                }
                if(isset($json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['MotResultsSummary']['MotResults'][0]['DateOfTest'])){
                   $json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['LastMotDate'] = date('d M Y', strtotime($json_return["Results"]["InitialVehicleCheckModel"]['BasicVehicleDetailsModel']['MotResultsSummary']['MotResults'][0]['DateOfTest']));
                }
            }
        }
    }
